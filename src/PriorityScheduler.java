public class PriorityScheduler extends Scheduler
{
    ...
    public void tick()
    {
        ...
        if (job != null)
        {
            higherPriority = cpu.readyQueue.first();
            if(higherPriority != null) {
                for (job = cpu.readyQueue.first(); job != null; job = job.next()) {
                    if (job.priority() > higherPriority.priority()) {
                        higherPriority = job;
                        job.enqueueStart(cpu.readyQueue);
                    }
                }
                job = cpu.currentJob.first();

                if (higherPriority.priority() > job.priority()) {
                    job.enqueueEnd(cpu.readyQueue);
                    higherPriority.enqueueStart(cpu.currentJob);
                }
            }
            job = cpu.currentJob.first();
            if (job.done()) {
